"""
Filesystem paths for the app, all derived from this file's own location so the
...
# The original HTML/CSS/JS prototype sits one level up - read-only, and only a
# last-resort fallback when an asset is missing.
...
# Streamlit serves <app dir>/static at app/static/... only when
# server.enableStaticServing is on in .streamlit/config.toml.
...
# Reference technique clips. Optional; the analysis page falls back to a still.
VIDEOS_DIR: Path = ASSETS_DIR / "videos"

MAIN_CSS: Path = STYLES_DIR / "main.css"
MAIN_JS: Path = SCRIPTS_DIR / "main.js"

# Loaded on the /analyse page only, on top of main.css / main.js.
ANALYSE_CSS: Path = STYLES_DIR / "analyse.css"
ANALYSE_JS: Path = SCRIPTS_DIR / "analyse.js"


def read_text(path: Path, default: str = "") -> str:
    """Read a UTF-8 text file, returning `default` when it isn't there."""
    try:
